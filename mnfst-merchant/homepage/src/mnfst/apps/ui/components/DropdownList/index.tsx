...
interface IDropdownListItemProps {
  ...
  isSelected?: boolean;
  onClick?: (props: IElementEventTarget) => void;
  name: string;
  value: any;
}

interface IDropdownListItemsClassNames {
  items: string;
}
...
interface IDropdownListItemsProps {
  ...
  children: React.ReactNode;
  className?: string;
  classNamesMap?: IClassNamesMap<IDropdownListItemsClassNames>;
  forceDropdownListPosition?: string;
  isOpened: boolean;
  onClickOutside?: () => void;
}

interface IDropdownListClassNames {
  items: string;
}

interface IDropdownListProps {
  className?: string;
  classNamesMap?: IClassNamesMap<IDropdownListClassNames>;
  disabled?: boolean;
  forceDropdownListPosition?: string;
  isOpened: boolean;
  ...
  name?: string;
  onBlur: () => void;
  onSelect: (target: IElementEventTarget) => void;
  value: any;
}
...
class DropdownListItemsOrigin extends React.Component<IDropdownListItemsProps> {
  ...
  public render() {
    const {
      alignment, autoWidth, children, className, classNamesMap = {}, forceDropdownListPosition, isOpened,
    } = this.props;
...
const DropdownList: React.SFC<IDropdownListProps> = (props) => {
  const { className, classNamesMap = {}, forceDropdownListPosition, isOpened, items, value, onBlur, onSelect } = props;
